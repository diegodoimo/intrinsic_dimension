...
# ----------------------------------------------------------------------------------------------
def return_id_scaling_gride(X, range_max=64, d0=0.001, d1=1000, eps=1e-7, mg_estimator = False):

    N = X.shape[0]
    max_rank = min(N, range_max)

    distances, dist_indices, mus, rs = _return_mus_scaling(X,
            range_scaling=max_rank, maxk = 30, mg_estimator=mg_estimator
        )
    if mg_estimator:
        rs_scaling = np.mean(rs, axis = 0)
        ids_scaling = 1/np.mean(mus, axis = 0)
        "error is not implemented"
        ids_scaling_err = np.zero_like(ids_scaling)
...
def _remove_zero_dists(distances):
    ...
    # ----------------------------------------------------------------------------------------------
def _mus_scaling_reduce_func(dist, start, range_scaling, maxk, mg_estimator = False):
...
def _return_mus_scaling(X, range_scaling, maxk=30, mg_estimator = False):
    ...
    """
    reduce_func = partial(
        _mus_scaling_reduce_func, range_scaling=range_scaling, maxk = maxk, mg_estimator = mg_estimator
    )
